...
{
    internal class EcomDestinationWriter
    {
            ...
            foreach (Mapping mapping in job.Mappings.Where(m => !_addedMappingsForMoveToMainTables.Contains(m)))
            {
                ...
                if (HasRowsToImport(mapping, out tempTablePrefix))
                {
                    if ((mapping.DestinationTable.Name == "EcomProducts" || mapping.DestinationTable.Name == "EcomGroups") && deleteProductsAndGroupForSpecificLanguage)
                    {
                        ...
                        DynamicwebBulkInsertDestinationWriter.DeleteExcessFromMainTable(mapping, extraConditions, sqlCommand, tempTablePrefix, removeMissingAfterImportDestinationTablesOnly);
                    }
                    else if (!(mapping.DestinationTable.Name == "EcomGroups" && !_removeFromEcomGroups) && !(mapping.DestinationTable.Name == "EcomVariantGroups" && !_removeFromEcomVariantGroups))
                    {
                        ...
                        {
                            DynamicwebBulkInsertDestinationWriter.DeactivateMissingProductsInMainTable(mapping, sqlCommand, shop, _defaultLanguageId, hideDeactivatedProducts);
                        }
                        else if (removeMissingAfterImport || removeMissingAfterImportDestinationTablesOnly)
                        {
                            DynamicwebBulkInsertDestinationWriter.DeleteExcessFromMainTable(mapping, GetExtraConditions(mapping, shop, null), sqlCommand, tempTablePrefix, removeMissingAfterImportDestinationTablesOnly);
                        }
                    }
                }
            }
        ...
        public void DeleteExistingFromMainTable(string shop, SqlTransaction transaction, string languageId)
        {
            ...
            foreach (Mapping mapping in job.Mappings)
            {
                ...
                if (HasRowsToImport(mapping, out tempTablePrefix))
                {
                    DynamicwebBulkInsertDestinationWriter.DeleteExistingFromMainTable(mapping, GetExtraConditions(mapping, shop, languageId), sqlCommand, tempTablePrefix);
                }
            }
        }
        ...
        private void MoveDataToMainTable(Mapping mapping, string tempTablePrefix, SqlTransaction sqlTransaction, bool updateOnly, bool insertOnly)
        {
            ...
            {
                throw GetMoveDataToMainTableException(ex, sqlCommand, mapping, tempTablePrefix, sqlTransaction);
            }
        }

        internal static Exception GetMoveDataToMainTableException(Exception ex, SqlCommand sqlCommand, Mapping mapping, string tempTablePrefix, SqlTransaction sqlTransaction)
        {
            string message = string.Format("failed to move data from temporary table [{0}.{1}{2}] to main table [{0}.{3}]. Exception: {4} Sql query: {5}",
                mapping.DestinationTable.SqlSchema,
                 mapping.DestinationTable.Name,
                tempTablePrefix,
                mapping.DestinationTable.Name,
                ex.Message, sqlCommand.CommandText);
            if (ex is SqlException)
            {
                message += GetSqlExceptionMessage(ex as SqlException, sqlCommand, mapping, tempTablePrefix, sqlTransaction);
            }
            return new Exception(message, ex);
        }

        private static string GetSqlExceptionMessage(SqlException exception, SqlCommand sqlCommand, Mapping mapping, string tempTablePrefix, SqlTransaction sqlTransaction)
        {
            ...
            if (exception.Number == 547)
            {
                ...
                if (mappings.Count > 0)
                {
                    List<FKTableConstraint> constraints = GetTableConstraints(sqlCommand, mapping.DestinationTable.Name, sqlTransaction);
                    List<string> columns = new List<string>();
                    foreach (var columnMapping in mapping.GetColumnMappings().Where(cm => cm.Active))
                    {
                        columns.Add("[" + columnMapping.DestinationColumn.Name + "]");
                    }
                    foreach (FKTableConstraint constraint in constraints)
                    {
                        foreach (ColumnMapping cm in mappings)
                        {
                            ...
                            {
                                var sql = string.Format("select {0} from [{1}].[{2}] where [{3}] not in (select distinct [{4}] from [{1}].[{5}])",
                                    string.Join(",", columns),
                                ...
                                using (var reader = Database.CreateDataReader(CommandBuilder.Create(sql), sqlCommand.Connection, sqlTransaction))
                                {
        ...
        private static List<FKTableConstraint> GetTableConstraints(SqlCommand sqlCommand, string fkTableName, SqlTransaction sqlTransaction)
        {
        ...
        private static bool HasIdentity(ColumnMappingCollection mappings)
        {
            return mappings.Any(cm => ((SqlColumn)cm.DestinationColumn).IsIdentity);
        }
    ...
    internal class FKTableConstraint
    {
